...
var ErrInvalidSource = errors.New("source must be a url, e.g postgresql://user:pass@localhost:5432/postgres")
...
func (b *PGPuppeteer) CreateUser(d, u string) (*Credentials, error) {
	...
	if !b.databaseExists(dbname) {
		return nil, fmt.Errorf("database '%s' doesn't exist", dbname)
	}
